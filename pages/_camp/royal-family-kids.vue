  ...
  <div class="component__wrap" v-if="pageData.campData">
		...
		<div class="content__wrap" v-for="(data, index) in pageData.campData" v-bind:key="index">
			<div class="cell cell__75">
				<div class="material__wrap --small --sharp__corners date__wrapper">
					<span>{{data.acf.camp_page_content.date_information.start_date}} - {{data.acf.camp_page_content.date_information.end_date}} </span>
				</div>
			</div>
		</div>		  
	  </div>	  
	  <div class="row">
		<div class="content__wrap" v-for="(data, index) in pageData.campData" v-bind:key="index">
			...
			<div class="cell cell__50">
				<article class="header__yellow paragraph__white paragraph__bold padding__left__5 padding__right__5" v-html="data.acf.camp_page_content.content_block"></article>
			</div>
		</div>		  
	</div>	 
	...
	<div class="row margin__bottom__5">
		<div class="content__wrap" v-for="(data, index) in pageData.campData" v-bind:key="index">
			<div class="cell cell__50">
				<article class="header__yellow paragraph__white paragraph__bold padding__left__5 padding__right__5" v-html="data.acf.camp_page_content.content_block_b"></article>
			</div>
		</div>
	</div>
	<div class="row margin__bottom__10">
		<div class="content__wrap --flex__end" v-for="(data, index) in pageData.campData" v-bind:key="index">
			<div class="cell cell__50">
				<article class="header__yellow paragraph__white paragraph__bold padding__left__5 padding__right__5" v-html="data.acf.camp_page_content.content_block_c"></article>
			</div>
		</div>
	</div>
...
export default {
	async asyncData({ store, params }) {
		let [camp, camps] = await Promise.all([
			axios.get(`${store.state.wordpressAPI}/wp/v2/lfh_camps/34`),
