<template>
  <div class="component__wrap our__mission__wrap padding__top__10" v-if="pageData.missionData">
    <div class="content__wrap --center" v-for="(componentData, index) in pageData.missionData" v-bind:key="index">
      <div class="cell cell__75">
		<div class="material__wrap">
			<article class="header__yellow paragraph__black" v-html="componentData.content.rendered"></article>
		</div>
      </div>
    </div>
		<div class="content__wrap --center padding__10" v-for="(componentData, index) in pageData.missionData" v-bind:key="index">
			<div class="cell cell__75">
				<article class="header__yellow paragraph__white paragraph__bold" v-html="componentData.acf.our_mission_page_content.content_block_b"></article>
			</div>			
		</div>
		<div class="content__wrap  --space__between margin__bottom__10">
			<div class="cell cell__25" v-for="(componentData, index) in pageData.missionData" v-bind:key="index">
				<div class="statistic__wrap material__wrap">
					<h1>{{componentData.acf.our_mission_page_content.statistics[0].statistic}}</h1>
					<h2>{{componentData.acf.our_mission_page_content.statistics[0].content}}</h2>
				</div>
			</div>
			<div class="cell cell__25" v-for="(componentData, index) in pageData.missionData" v-bind:key="index">
				<div class="statistic__wrap material__wrap">
					<h1>{{componentData.acf.our_mission_page_content.statistics[1].statistic}}</h1>
					<h2>{{componentData.acf.our_mission_page_content.statistics[1].content}}</h2>
				</div>
			</div>
			<div class="cell cell__25" v-for="(componentData, index) in pageData.missionData" v-bind:key="index">
				<div class="statistic__wrap material__wrap">
					<h1>{{componentData.acf.our_mission_page_content.statistics[2].statistic}}</h1>
					<h2>{{componentData.acf.our_mission_page_content.statistics[2].content}}</h2>
				</div>
			</div>
			<div class="cell cell__25" v-for="(componentData, index) in pageData.missionData" v-bind:key="index">
				<div class="statistic__wrap material__wrap">
					<h1>{{componentData.acf.our_mission_page_content.statistics[3].statistic}}</h1>
					<h2>{{componentData.acf.our_mission_page_content.statistics[3].content}}</h2>
				</div>
			</div>									
		</div>
		<div class="balloon__man__svg">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="2380.06 4371.789 54.285 177.434">
				<defs>
					<style>
						.cls-1 {
							fill: #c2c2c2;
